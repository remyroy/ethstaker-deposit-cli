...
def captive_prompt_callback(
    ...
    prompt_if_other_value: Optional[Tuple[str, Any]] = None,
    ...
    :param prompt_if_other_value: the optional tuple str of the other parameter to check and its comparing value.
    prompt if the source of the parameter is from the default value and the value of that other parameter equal to the
    comparing value
    '''
    def callback(ctx: click.Context, param: Any, user_input: str) -> Any:
        ...
        if (prompt_if_none and param.prompt is None
            ...
            user_input = click.prompt(prompt(), hide_input=hide_input, default=_value_of(default))
                ...
                and ctx.get_parameter_source(param.name) == click.core.ParameterSource.DEFAULT):
            user_input = click.prompt(prompt(), hide_input=hide_input, default=_value_of(default))
        elif (prompt_if_other_value is not None
                and ctx.params.get(prompt_if_other_value[0], None) == prompt_if_other_value[1]
                and ctx.get_parameter_source(param.name) == click.core.ParameterSource.DEFAULT):
            user_input = click.prompt(prompt(), hide_input=hide_input, default=_value_of(default))
